...
# Patched version of zsh4humans installer for unattended installation
...
if '[' '-n' "$rcs" ']'; then
  ...
  >&2 'printf' '\033[H\033[2J'
  >&2 'printf' 'You have the following Zsh startup files:\n'
  >&2 'printf' '\n'
  for f in $rcs; do
...
'trap' '"trap" "-" "INT" "TERM" "EXIT"; "command" "rm" "-f" "--" "$zshenv" "$zshrc" "$z4h" 2>/dev/null' \
  'INT' 'TERM' 'EXIT'
...
url='https://raw.githubusercontent.com/romkatv/zsh4humans/v4'
...
'set' '+ue'
'set' '--'
'export' ZSH="$HOME/.cache/zsh4humans/v4/ohmyzsh/ohmyzsh"
# '.' ~/'.zshenv'